...
def _lane_confidence_cap(*, lane_id: str, article_count: int, total_weight: float) -> int:
    ...
    if lane_id == "near_term":
        return hard_cap
    density_cap = 76 + min(article_count, 6) + round(min(total_weight, 4.0))
    return min(hard_cap, density_cap)


def _is_semis_symbol(metadata: dict[str, str]) -> bool:
    industry = str(metadata.get("industry", "")).lower()
    return "semi" in industry


def _relevance_weight(item: SymbolNewsItem, *, symbol: str, metadata: dict[str, str]) -> float:
    sector_scope = SECTOR_SCOPE_BY_NAME.get(str(metadata.get("sector", "")).strip(), "")
    if item.bucket == "symbol" or item.impact_scope == "symbol_specific":
        if symbol in item.relevance_symbols or not item.relevance_symbols:
            return 1.0
        return 0.0
    if item.bucket == "sector":
        if item.impact_scope == sector_scope:
            return 0.85
        if item.impact_scope == "semis" and _is_semis_symbol(metadata):
            return 0.85
    return 0.0
...
def build_symbol_news_rows(
    *,
    symbol_metadata: dict[str, dict[str, str]],
    model_symbols: list[str],
    interpretations_by_symbol: dict[str, list[SymbolNewsItem]],
    now_utc: datetime,
) -> list[dict]:
    rows: list[dict] = []
    ...
    for symbol in model_symbols:
        ...
        for lane_id in ("near_term", "swing_term", "position_term"):
            ...
            lane_items: list[tuple[SymbolNewsItem, float]] = []
            for item in symbol_items:
                if _lane_for_horizon(item.impact_horizon) != lane_id:
                    continue
                relevance = _relevance_weight(item, symbol=symbol, metadata=metadata)
                if relevance <= 0:
                    continue
                ...
                if recency_weight <= 0:
                    continue
                weight = relevance * recency_weight * _confidence_weight(item.confidence_label)
                lane_items.append((item, weight))

            article_count = len(lane_items)
            if article_count == 0:
                why_code = ["symbol_news_coverage_missing"]
            ...
            intensity = min(1.0, log1p(total_weight) / log1p(5.0))
            raw_score = direction_balance * intensity * 120.0
            ...
            evidence_density_score = min(100, round(article_count / 4 * 100))
            ...
            if article_count >= 2 and freshness_score >= 70:
            ...
            if signal_direction == "bullish":
                why_code.append("symbol_news_flow_bullish")
            elif signal_direction == "bearish":
                why_code.append("symbol_news_flow_bearish")
            else:
                why_code.append("symbol_news_flow_mixed")
            if article_count == 1:
                why_code.append("single_catalyst_only")
            elif article_count == 2:
                why_code.append("limited_catalyst_breadth")
